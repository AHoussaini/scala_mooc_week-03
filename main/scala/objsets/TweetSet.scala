...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet =  {
    def union0(that: TweetSet, accu: TweetSet): TweetSet = {
      ...
      else union0(that.tail, accu.incl(that.head))
    }
    union0(that, this)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    if (p(head))
      if (tail.isEmpty) accu.incl(head)
      else tail.filter0(p, accu.incl(head))
    else if (!tail.isEmpty) tail.filter0(p, accu)
    else accu
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(p => google.exists(s => p.text.contains(s)))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(p => apple.exists(s => p.text.contains(s)))
